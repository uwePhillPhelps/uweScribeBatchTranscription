...
repeat until (time_to_stop_script is true)
	...
	repeat until (count of items in selected_files) is greater than 0
		...
		set working_folder to choose folder with prompt "Please select folder containing audio files to transcribe" default location default_folder
		...
		-- if we can't find any audio files
		if (count of items in selected_files) is not greater than 0 then
			-- try looking inside split_wav sub folder
			set selected_files to {} -- create an empty list
			set working_folder to (working_folder as text) & "split_wav:"
			set working_folder to working_folder as alias
			
			-- try to find audio files inside the split_wav sub folder
			tell application "Finder"
				try
					set selected_files to every file of the working_folder whose name extension is in the extension_list
				end try
		...
		-- if we've still not found anything
		if (count of items in selected_files) is not greater than 0 then
			activate
	...
	activate application "MacSpeech Scribe"
	...
	-- by default we report existing rtf files
	set report_existing_rtf_files to true
	
	-- look at each file
	repeat with current_file in selected_files
		if ((name extension of current_file) as string) is in the extension_list then
			--then this is an audio file or an rtf file
			...
			tell application "Finder"
				-- if an rtf file with the same basename exists in the directory 
			...
			-- before beginning transcription, check existing rtf files 
			if exists_equivalent_rtf is true and report_existing_rtf_files is true then
				activate
				set button_returned to display dialog "An audio file already has a text transcript counterpart" & return & return & current_file_string & return & equivalent_rtf_filename buttons {"Report more existing counterparts", "Skip existing segments"}
				
				if button returned of button_returned contains "Skip" or button returned of button_returned contains "skip" then
					set report_existing_rtf_files to false
				else
					set report_existing_rtf_files to true
				end if
			else if exists_equivalent_rtf is false then
					...
					if button returned of button_returned contains "I want to hear the segment audio" then
						...
						tell application "Finder"
							if exists document file equivalent_rtf_pathname then
								...
								set exists_equivalent_rtf to false
								set dialog_preamble to "It does not look like you have finished editing this segment, the transcript text file counterpart could not be found " & return & return
							end if
						end tell
...
on invoke_open_dialog(app_name)
	...
	tell application "System Events"
		...
		-- menu item Transcription <elipsis unicode> É
	end tell
